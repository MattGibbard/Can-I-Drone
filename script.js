var DarkSkyKey = '6284670e428f60915bbecf052a0ae21b';
var varLatitude;
var varLongitude;
...
function loadWeather(click) {
    ...
    $.getJSON("https://query.yahooapis.com/v1/public/yql?q=" + searchtext + "&format=json").success(function (data) {
        ...
        $('#sunrise_placeholder').html('<div class="alert alert-warning" role="alert">' + "Sunrise: " + varSunrise + '</div>');
        $('#sunset_placeholder').html('<div class="alert alert-warning" role="alert">' + "Sunset: " + varSunset + '</div>');
        $('#humidity_placeholder').html('<div class="alert alert-primary" role="alert">' + "Humidity: " + varHumidity + "%" + '</div>');
        $('#pressure_placeholder').html('<div class="alert alert-primary" role="alert">' + "Pressure: " + varPressure + varPressureUnits + " (millibars)" + '</div>');
        ...
        $.getJSON("https://thingproxy.freeboard.io/fetch/https://api.darksky.net/forecast/" + DarkSkyKey + "/" + varLatitude + "," + varLongitude + "?units=si").success(function (data) {
            ...
            $('#DSStormDistance_placeholder').html('<div class="alert alert-secondary" role="alert">' + 'DarkSky - Nearest storm distance (Km): ' + varDSStormDistance + '</div>');
            $('#DSPrecipitationIntensity_placeholder').html('<div class="alert alert-secondary" role="alert">' + 'DarkSky - Precipitation Intensity (Millimeters per hour): ' + varDSPrecipitationIntensity + '</div>');
            $('#DSCloudCover_placeholder').html('<div class="alert alert-secondary" role="alert">' + 'DarkSky - Cloud Cover (percentage of sky occluded by clouds, between 0 and 1): ' + varDSCloudCover + '</div>');

            $('#DSSummary_placeholder').html('<div class="alert alert-secondary" role="alert">' + 'DarkSky - Summary: ' + varDSSummary + '</div>');
            $('#DSIcon_placeholder').html('<div class="alert alert-secondary" role="alert">' + 'DarkSky - Icon: ' + varDSIcon + '</div>');
            $('#DSTemperature_placeholder').html('<div class="alert alert-secondary" role="alert">' + 'DarkSky - Temperature (Degrees Celsius): ' + varDSTemperature + '</div>');
            $('#DSFeelsLike_placeholder').html('<div class="alert alert-secondary" role="alert">' + 'DarkSky - Feels like (Degrees Celsius): ' + varDSFeelsLike + '</div>');
            $('#DSHumidity_placeholder').html('<div class="alert alert-secondary" role="alert">' + 'DarkSky - Humidity % (0-1): ' + varDSHumidity + '</div>');
            $('#DSWindSpeed_placeholder').html('<div class="alert alert-secondary" role="alert">' + 'DarkSky - Wind Speed (Meters/s): ' + varDSWindSpeed + '</div>');
            ...
            $('#localinfo_placeholder').html('<div class="card text-white bg-secondary mb-3">\
                                                    ...
                                                    <div class="card-body">\
                                                        <h2 class="card-title">' + '<i class="wi wi-yahoo-' + varStatusCode + '"></i> ' + varStatusText + '</h2>\
                                                        <small class="card-text">Data at ' + varLastUpdate + '</small>\
                                                    </div>\
                                                </div>');
            ...
            $('#FlySafeScore_placeholder').html('<div class="card text-white bg-primary mb-3">\
                                                <div class="card-header">FlySafe Score</div>\
                                                    <div class="card-body">\
                                                        <h2 class="card-title">7 out of 10</h2>\
            ...
            if (varWindSpeed >= 20) {
                $('#windspeed_placeholder').html('<div class="card text-white bg-danger mb-3">\
                                                    <div class="card-header">Wind speed</div>\
                                                        <div class="card-body">\
                                                            <h5 class="card-title">' + varWindSpeed + varWindSpeedUnits + '</h5>\
                                                            <p class="card-text">Wind will affect the performance and safety of your drone.</p>\
                                                        </div>\
                                                    </div>');
            } else if (varWindSpeed < 20 && varWindSpeed > 15) {
            ...
            } else if (varTemperature <= 0 && varTemperature > -10) {
            ...
            if (varVisibility >= 10) {
                $('#visibility_placeholder').html('<div class="card text-white bg-success mb-3">\
                                                    <div class="card-header">Visibility</div>\
                                                        <div class="card-body">\
                                                            <h5 class="card-title">' + varVisibility + varVisibilityUnits + '</h5>\
                                                            <p class="card-text">Poor visibility will block the line of sight (LOS) to your drone.</p>\
                                                        </div>\
                                                    </div>');
            } else if (varVisibility < 10 && varVisibility > 5) {
            ...
            var myChart = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: ['rgba(149, 255, 227, 0.6)'],
                        borderColor: ['rgba(149,255,227,1)'],
                        borderWidth: 1
                    }]
                },
                options: {
